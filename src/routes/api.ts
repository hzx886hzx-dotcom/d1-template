...
function executeInternalStrategy(payload: StrategyPayload) {
  ...
  } else if (action === "max_fixed_solid_group") {
    const num = payload?.history?.[0] as LotteryResult;
    if (num === null || num === undefined) {
      return {
        period: Number(payload.period || 0),
...
export async function handleVerify(
  ...
  if (!/^[A-Z0-9]{4}(?:-[A-Z0-9]{4}){3}$/.test(sn)) {
    return json(400, {
      code: 400,
      msg: "sn must be XXXX-XXXX-XXXX-XXXX (uppercase letters/digits)",
    });
  }
  ...
  return json(200, {
    ...
    activation: {
      ...
      activatedAt: validated.record!.activated_at
        ? Number(validated.record!.activated_at)
        : null,
      expiresAt:
        validated.newExpiresAt || Number(validated.record!.expires_at || 0),
...
export async function handleGetScheme(
  ...
  const payload = readExternalToken(
  ...
  const usage = buildUsageContext(request, body);
  if (
    normalizeCode(payload.device_id) !==
    normalizeCode(usage.deviceId || "UNKNOWN")
  ) {
    return json(401, { code: 401, msg: "token device mismatch" });
  }

  const activeCheck = await checkCodeAvailableForScheme(
    db,
    payload.sn,
    normalizeCode(usage.deviceId || "UNKNOWN"),
  );
